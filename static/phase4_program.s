...
target:
  ...
  ; new code for Phase 4 starting here

    in R6           ; set 8 switches (SW[0] to SW[7]) to 0xE0 – read it into the lower 8 bits
    st 0x77, R6     ; of R6 (set other input bits to 0), and save it at memory location with address 0x77 for the next loop iteration
    ldi R3, 0x2E    ; address of "loop" in R3
    ldi R5, 1       ; R5 = 1
    ldi R2, 40      ; R2 = 40, loop counter (5 times display of 8 numbers in a loop)
 
 loop: 
    out R6           ; display the lower 8 bits of R6 on the two 7-segment displays
    ldi R2, -1(R2)   ; is the loop done?
    brzr R2, 8       ; yes – branch to done
    ld R7, 0x88      ; no – set R7 = 0xFFFF stored in memory address 0x88  ; delay, so you
 
 loop2: 
    ldi R7, -1(R7)   ; can see the numbers on the two 7-segment displays
    nop
    brnz R7, -3      ; branch to "loop2" if R7 ≠ 0 – delay is not done yet
    shr R6, R6, R5   ; delay is done - shift the number in R6 right once
    brnz R6, -9      ; back to "loop" and display the shifted number if it is not zero
    ld R6, 0x77      ; if it is zero, start over with the number 0xE0 stored at address 0x77
    jr R3            ; branch to "loop" using address in R3
 
 done: 
    ldi R6, 0x63     ; final display value 0x63
    out R6           ; display the final value 0x63 on the two 7-segment displays
  ; new code for Phase 4 ending here
